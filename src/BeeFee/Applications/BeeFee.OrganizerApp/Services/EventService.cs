...
namespace BeeFee.OrganizerApp.Services
{
	public class EventService : BaseBeefeeService
	{
		...
		public bool AddEvent(string categoryId, string name, string label, string url, string cover, EEventType type,
			EventDateTime dateTime,
			Address address, TicketPrice[] prices, string html)
			=> Insert(
				new NewEvent(Get<BaseUserProjection>(User.Id), Get<BaseCategoryProjection>(categoryId), name, label, url, cover, type,
					dateTime,
		...
		public bool UpdateEvent(string id, int version, string name, string label, string url, string cover, EventDateTime dateTime, Address address, EEventType type,
			string categoryId, TicketPrice[] prices, string html)
			=> Update<EventProjection>(id, version, x => x.Change(name, label, url, cover, dateTime, address, type, Get<BaseCategoryProjection>(categoryId.HasNotNullArg("category")), prices, html), true)
